$(() => {
    ...
    if(sessionStorage['login_role'] == "user") {
        $.get(
        ...
        .done((data, status, jqXHR) => {
            ...
            $.each(data, (i, data) => {
                ...
                if(data.status == "JÄTETTY" || data.status == "VASTATTU" || data.status == "HYVÄKSYTTY" || data.status == "HYLÄTTY") {
                    ...
                    if(data.status == "JÄTETTY") {
                        button = "<td><button class='edit_button' data-editid="+ data.order_id +">Vastaa</button></td><td><button class='delete' data-deleteid="+ data.order_id +">Poista</button></td>";
                    }
                    ...
                    // Is added to every edit button
                    $(".edit_button").click(function() {
                        $("#admin_edit_dialog").dialog("open");
                        var id = $(this).attr('data-editid');
                        workorder_id = id;
                        admineditWorkorder(id);
                    }); 
                }
            });
    ...
    function admineditWorkorder(id) {
        $.get(
            "http://localhost:3001/workorder/"+
            id
        )
        .done( (data, status, jqXHR) => {
            $("#admin_edit_work_desc").val( data[0].work_description );
            $("#admin_edit_address").val( data[0].address );
            $("#admin_edit_city").val( data[0].city);
        })
    }
