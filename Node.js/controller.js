...
module.exports = 
{
    ...
    fetchAll: (req, res) => {
        CONNECTION.query('SELECT * FROM users', (err, result, fields) => {
            if(err) {
                console.log("Error while trying to get all data from users-table, reason: " + err.sqlMessage);
    ...
    fetchOneUser: (req, res) => {
        ...
        CONNECTION.query('SELECT * FROM users WHERE username = ?', [username],
            (error, result, fields) => {
                if(error) {
                    console.log("Error while fetching user and password from user table, reason: " + error.sqlMessage);
    ...
    fetchHousingTypes: (req, res) => {
        CONNECTION.query('SELECT * FROM housing_types', 
            (error, result, fields) => {
                if(error) {
                    console.log("Error while trying to fetch housing types from housing_types-table, reason: "+error.sqlMessage);
                    res.json({"status": 500, "error": error, "response": null});
                } else {
    ...
    fetchWorkorder: (req, res) => {
        ...
        CONNECTION.query('SELECT * FROM workorders WHERE order_id=?', [id], 
            (error, results, fields) => {
                if(error) {
                    console.log("Erro while fetching data for edit dialog, reason: "+error.sqlMessage);
                    res.json({"status": 500, "error": error, "response": null});
                } else {
    ...
    fetchWorkorders: (req, res) => {
        ...
        CONNECTION.query('SELECT * FROM workorders w LEFT JOIN states s ON w.status = s.id WHERE w.order_username=?', [user], 
            (error, results, fields) => {
                if(error) {
                    console.log("Error while fetching workorders from workorders table, reason: " + error.sqlMessage);
                    res.json({"status": 500, "error": error, "response": null});
                } else {
    ...
    createUser: (req, res) => {
        ...
        CONNECTION.query('INSERT INTO users (username, password, name, visiting_address) VALUES (?, ?, ?, ?)', [v.username_reg, v.password_reg, v.name_reg, v.address_reg],
            (err, results, fields) => {
                if(err) {
                    console.log("Error while tried to add new user to users table, reason: "+err.sqlMessage);
                    res.json(err);
                } else {
    ...
    createWorkorder: (req, res) => {
        ...
        CONNECTION.query('INSERT INTO workorders (order_username, work_description, address, city, orderdate, status)  VALUES (?, ?, ?, ?, CURDATE(), ?)', [ v.add_username, v.add_info, v.add_address, v.add_city, v.add_status],
            (error, results, fields) => {
                if(error) {
                    console.log("Error while trying to add new workorder, reason: "+error.sqlMessage);
                    res.json(error);
                } else {
    ...
    deleteWorkorder: (req, res) => {
        ...
        CONNECTION.query('DELETE FROM workorders WHERE order_id=?', [key], 
            (error, results, fields) => {
                if(error) {
                    console.log("Error while trying to delete data from workorders-table ,reason: "+error.sqlMessage);
                    res.send(error);
                } else {
    ...
    deleteUser: (req, res) => {
        ...
        CONNECTION.query('DELETE FROM users WHERE username=?', [key], 
            (error, results, fields) => {
                if(error) {
                    console.log("Error while trying to delete data from users-table, reason: "+error.sql);
                    res.send(error);
                } else {
